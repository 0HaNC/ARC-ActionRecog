...
def main():
    ...
    is_online = args.dataset=='kinetics_online'
    train_loader = torch.utils.data.DataLoader(
        TSNDataSet(args.dataset, args.root_path, args.train_list, num_segments=args.num_segments,
                   ...
                   transform=torchvision.transforms.Compose([
                       ...
                       normalize,
                   ]), dense_sample=args.dense_sample, scale=args.scale, online_decode=is_online),
    ...
    output_list = []
    if args.evaluate:
        # no crop, keep resolution
        input_size = scale_size
        infor = args.suffix
        how2crop, timestamp = infor.split('_')[0], infor.split('_')[1]
        cropping = torchvision.transforms.Compose([
            GroupScale(model.module.scale_size),
            GroupFullResSample_component(input_size, component=how2crop, flip=False)])
        val_loader = torch.utils.data.DataLoader(
            TSNDataSet(args.dataset, args.root_path, args.val_list, num_segments=args.num_segments,
                       new_length=data_length,
                       modality=args.modality,
        ...
        #prec1, score_tensor = test(test_loader, model, criterion, 0, num_class=num_class)
        #prec1 = validate(val_loader, model, criterion, epoch, log_training, tf_writer, mixed, autocast)
        inference(val_loader, model, criterion, 0, mixed=False, autocast=autocast)
...
def inference(val_loader, model, criterion, epoch, log=None, tf_writer=None, mixed=True, autocast=None):
    batch_time = AverageMeter()
    data_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    ...
    try:
        ...
        pt_path = 'results_TSM-R18'
        if not osp.exists(pt_path):
            os.makedirs(pt_path)
        save_name = './{}/{}{}'.format(pt_path, how2crop, timestamp)
        while(os.path.exists(save_name)):
            timestamp += 1
            save_name = './{}/{}{}'.format(pt_path, how2crop, timestamp)
        torch.save(record, './{}/{}{}'.format(pt_path, how2crop, timestamp))
    except:
        set_trace()
...
def validate(val_loader, model, criterion, epoch, log=None, tf_writer=None, mixed=True, autocast=None):
    ...
    try:
        ...
        how2crop, timestamp = infor.split('_')[0], infor.split('_')[1]
        timestamp = int(timestamp)
        save_name = './pt_path/{}{}'.format(how2crop, timestamp)
        while(os.path.exists(save_name)):
            timestamp += 1
            save_name = './pt_path/{}{}'.format(how2crop, timestamp)
        torch.save(record, './pt_path/{}{}'.format(how2crop, timestamp))
    except:
        set_trace()
    '''
    output = ('Testing Results: Prec@1 {top1.avg:.3f} Prec@5 {top5.avg:.3f} Loss {loss.avg:.5f}'
...
def eval_video(video_data, num_class, net, this_test_segments, modality):
    #     net.eval()
    with torch.no_grad():
...
def test(val_loader, model, criterion, iter, num_class, logger=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    for i, (input, target) in enumerate(val_loader):
        ...
        i, output, label = eval_video((i,input_var,target_var), num_class, model,this_test_segments=args.num_segments, modality = args.modality)                
