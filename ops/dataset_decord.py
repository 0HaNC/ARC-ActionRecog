...
try:
    ...
    print('decord package not installed')
...
class TSNDataSet(data.Dataset):
    def __init__(self, dataset, root_path, list_file,
                 ...
                 remove_missing=False, dense_sample=False, twice_sample=False, scale=1):
    ...
    def _sample_indices(self, vid_num_frames):
        """
        ...
        if self.dense_sample:  # i3d dense sample
            sample_pos = max(1, 1 + vid_num_frames - 64)
            t_stride = 64 // self.num_segments
            start_idx = 0 if sample_pos == 1 else np.random.randint(0, sample_pos - 1)
            offsets = [(idx * t_stride + start_idx) % vid_num_frames for idx in range(self.num_segments)]
            return np.array(offsets) + 1
        else:  # normal sample
            average_duration = (vid_num_frames - self.new_length + 1) // self.num_segments
            if average_duration > 0:
                offsets = np.multiply(list(range(self.num_segments)), average_duration) + randint(average_duration,
                                                                                                  size=self.num_segments)
            elif vid_num_frames > self.num_segments:
                offsets = np.sort(randint(vid_num_frames - self.new_length + 1, size=self.num_segments))
            else:
                offsets = np.zeros((self.num_segments,))
            return offsets + 1

    def _get_val_indices(self, vid_num_frames):
        if self.dense_sample:  # i3d dense sample
            sample_pos = max(1, 1 + vid_num_frames - 64)
            t_stride = 64 // self.num_segments
            start_idx = 0 if sample_pos == 1 else np.random.randint(0, sample_pos - 1)
            ...
            if vid_num_frames > self.num_segments + self.new_length - 1:
                tick = (vid_num_frames - self.new_length + 1) / float(self.num_segments)
                offsets = np.array([int(tick / 2.0 + tick * x) for x in range(self.num_segments)])
            else:
                offsets = np.zeros((self.num_segments,))
            return offsets + 1

    def _get_test_indices(self, vid_num_frames):
        if self.dense_sample:
            sample_pos = max(1, 1 + vid_num_frames - 64)
            t_stride = 64 // self.num_segments
    ...
    def __getitem__(self, index):
        ...
        if('something' in self.dataset or 'diving' in self.dataset): 
            decode_boo = False
            # video_list is the list of all frms of the video
            ...
            try:
                ...
                video_list = decord.VideoReader(os.path.join(self.root_path, video_path))
        ...
        if not self.test_mode:
            segment_indices = self._sample_indices(len(video_list)) if self.random_shift else self._get_val_indices(len(video_list))
        else:
            segment_indices = self._get_test_indices(len(video_list))
