...
class TSN(nn.Module):
    def __init__(self, num_class, num_segments, modality,
                 ...
                 is_shift=False, shift_div=8, shift_place='blockres', fc_lr5=True,
                 temporal_pool=False, non_local=False, groups=1, shuffle=False, args=None, target_transforms = None):
    ...
    def _prepare_base_model(self, base_model):
        ...
        if 'res' in base_model:
            '''
            if self.groups>1 or 'CRes' in base_model or 'pw' in base_model or ('skip_connection' in self.args.shift_place) or 'wider' in base_model  :
                import archs
                self.base_model = getattr(archs, base_model)(True if self.pretrain == 'imagenet' else False,
                 groups=self.groups, args=self.args)
            ''' 
            if 'Mres' in base_model or 'Slow' in base_model or 'ada' in base_model:
                import archs
                # set groups=1
                self.base_model = getattr(archs, base_model)(True if self.pretrain == 'imagenet' else False,
                 groups=1, args=self.args)
            ...
            if self.is_shift:
                ...
                make_temporal_shift(self.base_model, self.num_segments,
                                    n_div=self.shift_div, place=self.shift_place, temporal_pool=self.temporal_pool,
                                     groups=self.groups, shuffle=self.shuffle, args=self.args)
            ...
            import archs
            self.base_model = getattr(archs, base_model)(pretrained=True, num_classes=self.num_class, drop_rate=self.args.vit_dropout, attn_drop_rate=self.args.vit_dropout)
            if self.is_shift:
                print('Adding temporal shift...')
                from ops.temporal_shift import make_temporal_shift
                make_temporal_shift(self.base_model, self.num_segments,
                                    n_div=self.shift_div, place=self.shift_place, temporal_pool=self.temporal_pool,
                                     groups=self.groups, shuffle=self.shuffle, args=self.args)
            self.base_model.blocks = self.base_model.blocks[:self.args.vit_depth]
    ...
    def get_optim_policies(self):
        ...
        for name, p in self.named_parameters():
            if re.search(r'channel_interacter.Weight$',name) or re.search(r'channel_interacter.Weight_mat*',name) or re.search(r'channel_interacter.net.Weight_mat*',name)\
             or re.search(r'channel_interacter.w$', name) or re.search(r'channel_interacter.b$', name)\
              or re.search(r'affiner.w$', name) or re.search(r'affiner.b$', name):
                #set_trace()
                custom_ops.append(p)
            if re.search(r'channel_interacter.dw_weights$',name):
        ...
        return [
            ...
            {'params': ln, 'lr_mult': 1, 'decay_mult': 0,
             'name': "LN scale"},
            {'params': custom_ops, 'lr_mult': 1, 'decay_mult': 0,
             'name': "custom_ops"},
            # for fc
    ...
    def forward(self, input, no_reshape=False):
        if not no_reshape:
            ...
            # once sample: input of size(batch, new_length*num_segments*num_channels_img, spatial_dim1, spatial_dim2)
            # twice sample: input of size(batch, new_length*num_segments*2*num_channels_img, spatial_dim1, spatial_dim2)
            base_out = self.base_model(input.view((-1, sample_len) + input.size()[-2:]))
            #base_out = self.base_model(input.view((-1, self.num_segments, 3*self.new_length) + input.size()[-2:]).contiguous().transpose(1, 2))
        else:
