...
class NL3DWrapper(nn.Module):
    def __init__(self, block, n_segment):
        ...
        try:
            ...
            self.nl = NONLocalBlock3D(block.bn2.num_features)
...
def make_non_local(net, n_segment):
    import torchvision
    import archs
    if isinstance(net, torchvision.models.ResNet) and len(net.layer3)>=6 and len(net.layer2)>=4:
    ...
    elif isinstance(net, torchvision.models.ResNet) and (len(net.layer3)<6 or len(net.layer2)<4):
        net.layer2 = nn.Sequential(
            NL3DWrapper(net.layer2[0], n_segment),
            NL3DWrapper(net.layer2[1],n_segment)
        )
        net.layer3 = nn.Sequential(
            NL3DWrapper(net.layer3[0], n_segment),
            NL3DWrapper(net.layer3[1],n_segment)
        )
    else:
