...
class GroupNdArrToPIL(object):
	# input list of PIL.Images, output list of torch tensor
	def __init__(self):
		pass
	#@timeit
	def __call__(self, arr_lab):
		arr, label = arr_lab
		# a of shape(H,W,C)
		# set_trace()
		return [Image.fromarray(a) for a in arr], label
...
class GroupFullResSample_component(object):
    def __init__(self, crop_size, scale_size=None, flip=True, component=None):
        self.crop_size = crop_size if not isinstance(crop_size, int) else (crop_size, crop_size)
    ...
    def __call__(self, img_tuple):
        ...
        offsets = list()
        if self.component=='left':
            offsets.append((0 * w_step, 2 * h_step))  # left
        elif self.component=='right':
            offsets.append((4 * w_step, 2 * h_step))  # right
        elif self.component=='center':
            offsets.append((2 * w_step, 2 * h_step))  # center
        else:
            raise ValueError(self.component)
