...
class TSN(nn.Module):
    ...
    def CAM(self, input, no_reshape=False):
        if not no_reshape:
            ...
            CAM_, x = self.base_model.CAM(input.view((-1, sample_len) + input.size()[-2:]))

        else:
            CAM_, x = self.base_model.CAM(input)
        #import pdb;pdb.set_trace();
        if self.dropout > 0:
             x = self.new_fc(x)
             val, ind = x.mean(dim=0).topk(1, dim=-1)
             #import pdb;pdb.set_trace();
             CAM_ = self.new_fc(CAM_.permute(0, 2, 3, 1))[:,:,:,ind[0]:ind[0]+1].permute(0, 3, 1, 2)
        CAM_ = CAM_ - CAM_.min(-1,keepdim=True)[0].min(-1,keepdim=True)[0]
        CAM_ = CAM_ / CAM_.max(-1,keepdim=True)[0].max(-1,keepdim=True)[0]
        ...
        if self.reshape:
            if self.is_shift and self.temporal_pool:
                ...
                CAM_ = CAM_.view((-1, self.num_segments) + CAM_.size()[-3:])
