...
def main():
    ...
    args.store_name += '_{where2insert}-{insert_propotion}-{layer_info}-{interacter}-{with_bn}'.format(
        layer_info=args.layer_info, where2insert=args.where2insert, 
        insert_propotion=args.insert_propotion, interacter=args.interacter, with_bn=args.with_bn)
    if args.suffix is not None:
        args.store_name += '_{}'.format(args.suffix)    
    args.store_name = args.store_name + '_{time}'.format(time=datetime.now().strftime('%d_%B_%Y_%Hh_%Mm_%Ss'))
    if args.resume:
    ...
    elif 'jester' in args.dataset:
        target_transforms = {0: 1, 1: 0, 6: 7, 7: 6}
    else:
    ...
    train_loader = torch.utils.data.DataLoader(
        TSNDataSet(args.dataset, args.root_path, args.train_list, num_segments=args.num_segments,
                   ...
                   transform=torchvision.transforms.Compose([
                       ...
                       normalize,
                   ]), dense_sample=args.dense_sample, scale=args.scale, args=args),
    ...
    if args.evaluate:
        ...
        test_loader = torch.utils.data.DataLoader(
            ...
            batch_size=args.batch_size, shuffle=False,
            num_workers=args.workers, pin_memory=True)
        test(val_loader, model, criterion, 0, mixed=False, autocast=autocast)
        return
...
def train(train_loader, model, criterion, optimizer, epoch, log, tf_writer, mixed, autocast, scaler):
    ...
    'module.base_model.layer1.0.conv2.channel_interacter.Weight_mat2',
    ...
    check_list += [i.replace('conv2', 'conv1') for i in check_list]
    if args.suffix and 'checkPretrain' in args.suffix:
    ...
    if 'SlowOnly' in args.arch and args.T_ARC:
        check_list += [i.replace('conv2', 'conv1') for i in check_list]
    if hasattr(args, 'suffix') and 'ShiftPerConv' == args.suffix:
        check_list = check_list + [i.replace('channel_interacter', 'channel_interacter.net') for i in check_list]
    for name, param in model.named_parameters():
        #print(name)
        if name in check_list or (re.search(r'.*.0.conv2.channel_interacter.Weight_mat*',name)) or (re.search(r'.*.0.conv1.channel_interacter.Weight_mat*',name)) or (re.search(r'.*.0.conv2.net.channel_interacter.net.Weight_mat*',name)):
            if param.dim() != 0:
                output = '{}: abs_max:{:.6f} abs_mean:{:.6f}'.format(name, param.detach().abs().cpu().max().item(),
                    param.detach().abs().cpu().mean().item())
                print(output)
                if log is not None:
                    log.write(output + '\n')
                    log.flush()
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        with autocast():
            ...
            # when autocast is enabled, input_var.dtype==torch.float32, output.dtype==torch.float16, 
            # set_trace()
            '''
            print(profile)
        ...
        if i % args.print_freq == 0:
            ...
            for name, param in model.named_parameters():
                #print(name)
                if name in check_list or (re.search(r'.*.0.conv2.channel_interacter.Weight_mat*',name)) or (re.search(r'.*.0.conv2.net.channel_interacter.net.Weight_mat*',name)):
                    if param.dim() != 0:
                        output = '{}: abs_max:{:.6f} abs_mean:{:.6f}'.format(name, param.detach().abs().cpu().max().item(), 
                            param.detach().abs().cpu().mean().item())
                        print(output)
            ...
            '''
            '''
            print(model.module.base_model.layer2[1].conv2.channel_interacter.cache_attend,
            model.module.base_model.layer2[1].conv2.channel_interacter.cache_attend.view(-1).abs().mean().item(),
             'hhhh')
            '''
            #sd = model.state_dict()
            #set_trace()
            ...
            output = ('Epoch: [{0}][{1}/{2}], lr: {lr:.5f}\t'
                ...
                data_time=data_time, loss=losses, scale=scaler.get_scale(), scaled_Loss=scaler.scale(loss).item(), top1=top1, top5=top5, lr=optimizer.param_groups[-3]['lr']))  # TODO
            print(output)
            log.write(output + '\n')
            log.flush()
        #exit()
...
def test(val_loader, model, criterion, epoch, log=None, tf_writer=None, mixed=True, autocast=None):
    batch_time = AverageMeter()
    losses = AverageMeter()
    top1 = AverageMeter()
    top5 = AverageMeter()
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(val_loader):
            ...
            with autocast():
                # compute output
                output = model(input)
                output_rec.append(output.cpu())
                #set_trace()
                loss = criterion(output, target)
            #set_trace()
            video_pred.extend(output.argmax(-1).cpu().numpy())
            video_labels.extend(target.cpu().numpy())
            # measure accuracy and record loss
            prec1, prec5 = accuracy(output.data, target, topk=(1, 5))
            ...
            if i % args.print_freq == 0:
                ...
                if log is not None:
                    log.write(output + '\n')
                    log.flush()
        record=torch.cat(output_rec, dim=0)
        set_trace()  
